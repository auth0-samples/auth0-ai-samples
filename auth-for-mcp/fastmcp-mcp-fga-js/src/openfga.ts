 ...
 * 4. Create tuples in FGA to grant users access to tools:
...
export async function getTools(userId: string): Promise<string[]> {
  try {
    ...
    const response = await fgaClient.batchCheck(
      { checks },
      {
        // We are using higher consistency because when testing the MCP, you'll want changes in Auth0FGA to be reflected immediately. 
        // In production, you might choose ConsistencyPreference.MinimizeLatency for better performance.
