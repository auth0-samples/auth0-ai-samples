...
import { getTools } from "./openfga.js";
...
export const authenticate = async (
  ...
  try {
    ...
    // TODO: Note that the authenticate method is called twice per request, so the FGA Call will happen
    // twice, which is not ideal for performance. Caching could be implemented if needed.
    
    let availableTools = await getTools(decoded.sub);
