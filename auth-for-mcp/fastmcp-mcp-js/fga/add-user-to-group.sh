...
# 
# This script retrieves an Auth0 user by email and adds them to an FGA group.
# 
# Prerequisites:
...
if [ $# -ne 2 ]; then
  ...
  echo "  3. Create a tuple in FGA: user:<user_id> -> member -> group:<group-name>"
  exit 1
fi
...
echo -e "${BLUE}📝 Adding user ${USER_ID} to group: ${GROUP_NAME}${NC}"

# Write tuple to FGA
fga tuple write --store-id "$FGA_STORE_ID" \
  "user:${USER_ID}" member "group:${GROUP_NAME}" --on-duplicate ignore

if [ $? -eq 0 ]; then
  echo -e "${GREEN}✅ Successfully added user to group!${NC}"
