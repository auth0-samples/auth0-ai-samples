"""
Auth0 integration for MCP server with On-Behalf-Of Token Exchange support.

This module provides Auth0 authentication and authorization for MCP servers,
including token verification, middleware, scoped tool decorators, and on-behalf-of token exchange.
"""
...
class Auth0Mcp:
    """
    Auth0 integration for FastMCP servers with On-Behalf-Of Token Exchange support.

    Provides authentication middleware, authorization decorators,
    OAuth2 Protected Resource metadata endpoints, and on-behalf-of token exchange for MCP servers.
    ...
    Args:
        ...
        client_id: Auth0 client ID for OBO (optional)
        client_secret: Auth0 client secret for OBO (optional)
        mcp_server_url: Base URL of the MCP server (optional)
