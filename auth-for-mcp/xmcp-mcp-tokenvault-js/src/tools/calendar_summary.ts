...
import { getAuthInfo, getClient } from "@xmcp-dev/auth0";
import { google } from "googleapis";
import { createGoogleAuthClient } from "../google-auth";

/**
 * Schema definition for calendar_summary tool parameters, following the XMCP tool export convention.
...
/**
 * Metadata for the calendar_summary tool, following the XMCP tool export convention.
 */
...
export default async function calendarSummary(_params: InferSchema<typeof schema>) {
  ...
  // Exchange Auth0 token for Google access token using Token Vault
  const googleTokenSet = await client.getAccessTokenForConnection({
