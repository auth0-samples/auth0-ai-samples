...
import { exchangeTokenOnBehalfOf } from "./auth0.js";
...
export function registerTools(mcpServer: FastMCP<FastMCPAuthSession>) {
  ...
  mcpServer.addTool({
    name: "greet",
    description:
      "Greet a user with personalized authentication information retrieved from an upstream API using On-Behalf-Of Token Exchange.",
    annotations: {
      title: "Greet User with On-Behalf-Of Token Exchange",
      readOnlyHint: true,
    },
    parameters: z.object({
      name: z
    ...
    execute: async (args, { session: authInfo }) => {
      ...
      // Perform On-Behalf-Of Token Exchange and call upstream API
