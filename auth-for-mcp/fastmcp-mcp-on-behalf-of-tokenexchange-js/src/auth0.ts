...
export async function exchangeTokenOnBehalfOf(accessToken: string) {
    // Use the resource server's OAuth 2.0 client to exchange tokens on behalf of the user
    // The 'audience' parameter specifies the target downstream API for the exchanged token
    return await apiClient.getTokenOnBehalfOf(accessToken, {
      audience: API_AUTH0_AUDIENCE, // Target audience for the downstream API
